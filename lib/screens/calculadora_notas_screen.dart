...
class _CalculadoraNotasScreenState extends State<CalculadoraNotasScreen> {
  Map<String, dynamic> _calcularEstado() {
    ...
    double porcentajeFaltante = 100.0 - porcentajeEvaluado;
    ...
    double promedioParcial =
        porcentajeEvaluado > 0 ? (notaAcumulada / (porcentajeEvaluado / 100)) : 0.0;
  ...
  Widget build(BuildContext context) {
    final color = widget.ramo.color;
    final estado = _calcularEstado();
    final notaNecesaria = estado['notaNecesaria'] as double;
    final porcentajeFaltante = estado['porcentajeFaltante'] as double;
    final promedioParcial = estado['promedioParcial'] as double;

    return Scaffold(
      appBar: AppBar(
        title: Text('Notas - ${widget.ramo.nombre}'),
        ...
        children: [
                ...
                Row(
                  ...
                  children: [
                    _PanelEstadistica(
                      ...
                      valor: promedioParcial.toStringAsFixed(1),
                      color: promedioParcial >= 4.0 ? color : Colors.red,
                    ),
                    _PanelEstadistica(
                      titulo: 'Nota a Presentación',
                      valor: estado['notaAcumulada'].toStringAsFixed(1),
                ...
                Container(
                  ...
                  child: Text(
                    porcentajeFaltante <= 0
                        ? '¡Semestre cerrado! Ya tienes el 100% de las notas.'
                        : notaNecesaria <= 1.0
                            ? '¡Ya pasaste el ramo! Estás al otro lado.'
                            : notaNecesaria > 7.0
                                ? 'Lo siento hermano ya fue'
                                : 'Necesitas un ${notaNecesaria.toStringAsFixed(1)} en el ${porcentajeFaltante.toStringAsFixed(0)}% restante para el 4.0',
                    style: TextStyle(
          ...
          Expanded(
            child: widget.ramo.usaSistemaRA
                ? (widget.ramo.gruposRA.isEmpty
                    ? const Center(child: Text('No hay RAs registrados.'))
                    : _construirListaRA(notaNecesaria, color))
                : (widget.ramo.evaluaciones.isEmpty
                    ? const Center(child: Text('No hay evaluaciones registradas.'))
                    : _construirListaNormal(notaNecesaria, color)),
          ),
        ],
      ),
    );
  ...
  Widget _construirListaNormal(double notaNecesaria, Color color) {
    return ListView.builder(
      padding: const EdgeInsets.all(16),
      itemCount: widget.ramo.evaluaciones.length,
      itemBuilder: (context, index) =>
          _tarjetaEvaluacion(widget.ramo.evaluaciones[index], notaNecesaria, color),
    );
  }

  Widget _construirListaRA(double notaNecesaria, Color color) {
    return ListView.builder(
      padding: const EdgeInsets.all(16),
      ...
      itemBuilder: (context, indexRA) {
        ...
        double notaAcumuladaRA = 0.0;
        double porcentajeEvaluadoRA = 0.0;
        for (var eval in ra.evaluaciones) {
          if (eval.notaObtenida != null) {
            notaAcumuladaRA += (eval.notaObtenida! * eval.porcentaje) / 100;
            porcentajeEvaluadoRA += eval.porcentaje;
          }
        }
        double promedioRA =
            porcentajeEvaluadoRA > 0 ? (notaAcumuladaRA / (porcentajeEvaluadoRA / 100)) : 0.0;
          ...
          child: Column(
            ...
            children: [
              Container(
                ...
                child: Row(
                  ...
                  children: [
                    Expanded(
                      child: Text(
                        '${ra.nombre} (Vale un ${ra.porcentaje}% final)',
                        style: TextStyle(fontWeight: FontWeight.bold, color: color),
                      ),
                    ),
                    if (porcentajeEvaluadoRA > 0)
                      Container(
                        padding: const EdgeInsets.symmetric(horizontal: 8, vertical: 4),
                        decoration: BoxDecoration(
                          color: promedioRA >= 4.0 ? color : Colors.red,
                          borderRadius: BorderRadius.circular(8),
                        ),
                        child: Text(
                          promedioRA.toStringAsFixed(1),
              ...
              Padding(
                padding: const EdgeInsets.all(12),
                child: Column(
                  children: ra.evaluaciones
                      .map((eval) => _tarjetaEvaluacion(eval, notaNecesaria, color,
                          subtextoExtra: '% de este RA'))
                      .toList(),
                ),
              ),
            ],
  ...
  Widget _tarjetaEvaluacion(Evaluacion eval, double notaNecesaria, Color color,
      {String subtextoExtra = '%'}) {
    return Card(
              ...
              child: Column(
                ...
                children: [
                  ...
                  Wrap(
                    ...
                    children: [
                      Text('Vale un ${eval.porcentaje}$subtextoExtra',
                      ...
                      if (eval.notaObtenida == null &&
                        ...
                        Container(
                          ...
                          child: Text(
                            'Meta: ${notaNecesaria.toStringAsFixed(1)}',
                            style: TextStyle(
                                fontSize: 12,
                                color: color,
            ...
            Expanded(
              ...
              child: TextFormField(
                ...
                onChanged: (valor) {
                  ...
                  setState(() {
                    eval.notaObtenida =
                        valorLimpio.isEmpty ? null : double.tryParse(valorLimpio);
                  });
                  widget.ramo.save();
                },
              ),
